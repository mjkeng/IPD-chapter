...
m_cost <- lm(cost_disc ~ arm, data = df_analysis)
summary(m_cost)$coefficients

# modelling difference in utilities
m_qol <- lm(qaly_disc ~ arm, data = df_analysis)
summary(m_qol)$coefficients

```

The models suggest an incremental difference of `r round(as.numeric(m_cost$coefficients["(arm)"]))` pounds and `r round(as.numeric(m_qol$coefficients["(arm)"]), digits = 2)` QALYs between the arms, which can be extracting using the code `round(as.numeric(m_cost$coefficients["(arm)"]))` and `round(as.numeric(m_qol$coefficients["(arm)"]), digits = 2)` respectively.

_Note._ Results from such unadjusted models could also be extracted using one the following methods: 

* By applying two-sampled t-tests using the `t.test` command:

```{r unadjustedTTest}

# difference in costs
t.test(cost_disc ~ arm, data = df_analysis)

# difference in QALYs
t.test(qaly_disc ~ arm, data = df_analysis)
...
df_analysis %>%
  group_by(arm) %>%
  summarise(mean(qaly_disc), mean(cost_disc))
...
m_cost <- lm(cost_disc ~ arm + sex + age, data = df_analysis)
summary(m_cost)$coefficients

# modelling difference in utilities (adjusted for age & gender)
m_qol <- lm(qaly_disc ~ arm + sex + age + qol_disc_0, data = df_analysis)
summary(m_qol)$coefficients
...
t.test(qol_disc_0 ~ arm, data = df_analysis)
...
First we need to summarise cost and utility differences by arm: __Or should this be done via the m_cost and m_qol models?__
...
df_inc <- df_analysis %>%
  ...
  summarise(cost_disc = mean(cost_disc), qaly_disc = mean(qaly_disc),
            .groups = 'drop') %>%
  # reshape into a wide format
  pivot_longer(cols = c("cost_disc", "qaly_disc")) %>%
  pivot_wider(names_from = c(name, arm), values_from = value) %>%
  # calculate incremental cost and utilities
  mutate(cost_inc = cost_disc_1 - cost_disc_0,
         qaly_inc = qaly_disc_1 - qaly_disc_0
  )
...
__Discussion point: perhaps use a package here, eg boot?__
...
_Note_ The above calculations are performed in a sequential loop but are independent of each other. It is therefore possible to parallelise the code, e.g. using the `foreach` command. __should we provide the code or is this  becoming too technical?__
...
We are now ready to generate confidence intervals: __To rewrite as a table; will make it shorter but also illustrate how to produce a table__
...
# in usual care arm
get_bootstrap_ci(df_boot$cost_disc_0)
# in intervention arm
get_bootstrap_ci(df_boot$cost_disc_1)
# incremental
get_bootstrap_ci(df_boot$cost_inc)

### utility differences ###

# in usual care arm
get_bootstrap_ci(df_boot$qaly_disc_0)
# in intervention arm
get_bootstrap_ci(df_boot$qaly_disc_1)
# incremental
get_bootstrap_ci(df_boot$qaly_inc)

### net benefit ###

# Net benefit at £20,000 threshold
get_bootstrap_ci(df_boot %>% filter(r == 20000) %>% select(nb) %>% pull())
# Net benefit at £30,000 threshold
get_bootstrap_ci(df_boot %>% filter(r == 30000) %>% select(nb) %>% pull())
