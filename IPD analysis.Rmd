...
___To add introduction paragraph about IPD___
...
<!-- mjk: Is it possible to match the number of participants in each arm with the actual trial (currently n = 265 for tx; n = 272 for placebo in synthetic data) -->
...
<!-- I think better to look at missing data pattern within missing data chapter? packages/functions for dealing with missing data possibly introduced in that chapter -->


## Descriptive statistics 

We first start by producing descriptive statistics for the dataset. 
This can include sample size, measures of central tendancy (mean, median, mode), measures of variability (standard deviation, variance), dispersion of data (minimum, maximum, interquatile range), shape of distribution of data (skewness, kurtosis), measures of dependence between variables (correlation, covariance). 
...
Examples of packages for producing descriptive statistics include: 
...
.
It is also possible to use base R or tidyverse to produce descriptive statistics, which probably gives you maximum control over how you want to summarise and present the data. 

### Baseline demographics 

Here, we summarise participant demographics at baseline, including participants sex, age, and their BMI category `bmicat`, separately for participants in the 10TT arm and the usual care arm.

<!-- mjk: add definition of BMI cat-->
...
Participants completed the EQ-5D-3L questionnaire during study visits at baseline and 3, 6, 12, 18 and 24 months. These responses were mapped onto utility scores based on the UK tariffs [@addref21] (labelled `qol_X`, where `X` is the month of measurement).
...
<!-- mjk: i assume costoth have already accounted for discounting & costint incurred in first year so no discounting needed. What about gpvis? The way totalcost is calculated now assumes all GP visits in first year. Possibly split gpvis into different years? also suggest removing total cost from raw data to demonstrate the calculation with discounting. -->
...
We now bring together all the datasets required for analysis. 
After excluding participants with missing data, 167 participants (31%) are included in our analysis, with 67 and 100 participants in the 10TT and usual care arms respectively.
...
Distribution of cost and QALYs by treatment arm 
