...
dput(samples,"samples0.Rdata")
```

```{r,echo=FALSE}
samples <- dget("samples0.Rdata")
```
...
dput(samples2,"samples2.Robject")
```
```{r,echo=FALSE}
samples2 <- dget("samples2.Robject")
```
```{r}
summary(samples2)
samples2[[1]][1:10,]
...
plot(results$Incremental_QALYs,results$Incremental_Costs,ylim=c(-2000,2000),xlim=c(-0.3,0.3),pch=16,xlab="Incremental QALYs",ylab="Indremental Costs")
